...
PORT = int(os.getenv('PORT', 30213))

# Register actor when fastapi starts up
from app import make_hobbit_agent, start_hobbit_agent, stop_hobbit_agent

@asynccontextmanager
async def lifespan(app: FastAPI):
    agent = make_hobbit_agent()
    await start_hobbit_agent(agent)
    app.state.hobbit_agent = agent
    yield
    await stop_hobbit_agent(agent)
